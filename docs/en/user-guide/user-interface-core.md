...
The user interface (UI) includes all visible elements users see and interact with while using KennerCore. This article contains the overview of all the main elements of user interface, which are the same for all entities in the system, as well as gives details on how to perform common tasks or operations in the system.
...
The following elements make up the KennerCore user interface:

- [**Navigation Menu**](#navigation-menu) – to navigate through available entities.
- [**Taskbar**](#taskbar) – to help the user interact with the KennerCore system in a more efficient way; additional information or actions are displayed here. 
- [**Search and Filtering Panel**](./search-and-filtering-core.md) – to search and filter within one entity or its related entities.

Within the KennerCore system your information may be presented in various ways:
...
The navigation menu in the default KennerCore theme is the menu placed on the left side of your user interface. It allows you to open list views for all entities in the system.
...
Taskbar is a top panel of the KennerCore user interface:
...
You can [search](./search-and-filtering-core.md) all the records existing in the KennerCore system using the global search functionality. Use the search form on the taskbar for it:
...
KennerCore uses two types of notifications: email notifications and system notifications (desktop notifications). Generally, email and desktop notifications are configured globally, for all users.
...
In the Queue Manager pop-up, the most recent KennerCore operations, which were executed in the background mode, are displayed. Additionally, the information on the data import performed with the help of import feeds is shown in the Queue Manager pop-up. To learn more about the ["Import Feeds"](https://treopim.com/store/import-feeds) module, please, visit our store.

To view the details of the performed operations, click `View List` in the upper right corner of the Queue Manager.

### Quick Create

To create new records from anywhere, click the quick create button located in the upper right corner of the taskbar on any KennerCore page and choose the entity for the record to be created:
...
On the user profile page, click the `Access` button to view your current permissions within the KennerCore system on the scope and field levels in the pop-up window that appears:
...
If you forget your password, it can be recovered using the `Forgot Password?` link on the KennerCore login page.
...
Recent updates and comments about certain entity records are displayed on the activity stream panel, making it an excellent way to keep track of the changes in KennerCore. 
...
The same options are available on the KennerCore starting page:
...
Alternatively, you can customize your [dashboard](./dashboards-and-dashlets-core.md#dashboards) view in the same way on the KennerCore starting page.

#### Email Accounts

KennerCore works with IMAP mail accounts, with or without SSL protection. KennerCore is able to send emails via SMTP Server, also with or without SSL protection, which can be enabled or not.
...
Select the `Last Viewed` option from the user drop-down menu to see the history of your visited pages within the KennerCore system:

![Last viewed](../../_assets/user-interface/last-viewed.jpg)

### Clear Local Cache

KennerCore is a single page application (SPA). To improve the page loading time, it actively uses cache for content as well as for the user interface.
...
KennerCore comes with breadcrumb navigation on each page in the system. Breadcrumb navigation is a form of a path-style clickable navigation, which links the user back to the prior website page in the overall online route. It reveals the path the user took to arrive to the given page. The `>` symbol separates out the hierarchical search order from beginning to end and may look something like:
